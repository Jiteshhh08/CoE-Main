...
export default function EventDetailClient({
  ...
  useEffect(() => {
    ...
    let b = mm ? mm[2] : uid;
    const mp: Record<string,string> = { CSECSA:'ECS',CSECSB:'ECS',CSECSC:'ECS',CSECS:'ECS',CSEA:'CSE',CSEB:'CSE',CSEC:'CSE',COMP:'COMP',IT:'IT',CSE:'CSE',AIML:'AIML',AIDS:'AIDS',ECS:'ECS',EXTCA:'ENTC',EXTCB:'ENTC',ENTCB:'ENTC',ENTCA:'ENTC',EXTC:'ENTC',ENTC:'ENTC',EXT:'ENTC',MME:'MME',MECH:'MECH',CIVIL:'CIVIL',BVDSD:'BVOC',BVSDE:'BVOC',BVOC:'BVOC',MCA:'MCA',BCA:'BCA',IOT:'IOT' };
                ...
                <div className="mt-4 overflow-x-auto">
                  <div className="flex flex-wrap items-center gap-2 border-b border-outline-variant/60 py-2">
                    ...
                    <select className="border border-outline-variant bg-white px-2 py-1 text-xs" value={leaderDept} onChange={(e) => setLeaderDept(e.target.value)}>
                      ...
                      {['COMP','IT','CSE','AIML','AIDS','ECS','ENTC','MECH','CIVIL','BVOC','MCA','BCA','IOT'].map((d) => (<option key={d} value={d}>{d}</option>))}
                    </select>
